...
using System.Collections;
	...
	{	
		...
		/// Basically there are 6 types of data types that are normally embedded in an image. 
		/// We take each one and treat them differently. 
		/// Sometimes in a specific data type we have to do 
		/// extra conversion for a specific tag (i.e. iso), 
		/// we have to perform specific calculations on the data after which we will 
		...
		public class ExifExtractor : IEnumerable
		{
			...
			public string this[string index]
			{
				get
				{
					if ( m_ExifData != null && m_ExifData.ContainsKey(index))
						return m_ExifData[index];
					return "";
				}
			}
			//
			...
			//
			private ExifTools m_ExifKeys;
			//
			private Dictionary<string, string> m_ExifData;
			//
			internal int Count
			{
				get
				{
					return this.m_ExifData.Count;
				}
			}
			//
			string sp;
			...
			public void setTag(int id, int len, short type, byte [] data)
			{
				...
				this.m_Image.SetPropertyItem( p );				
				buildDB(this.m_Image.PropertyItems);
			}
			/// <summary>
			/// 
			/// </summary>
			...
			{ 
				...
				// Copies the m_Data to the property item.
				item.Type = type;
				item.Len = len;
				item.Id = tag;
			...
			private void buildDB(System.Drawing.Imaging.PropertyItem[] parr)
			{
				...
				{
					string v = "";
					string name = (string)m_ExifKeys[p.Id];
					...
					if (p.Type == ExifDataType.Byte)
					{
						v = p.Value[0].ToString();
					}
						//2 = ASCII An 8-bit byte containing one 7-bit ASCII code. The final byte is terminated with NULL.,
					else if (p.Type == ExifDataType.String)
					{
						// string					
						v = ascii.GetString(p.Value);
					}
						//3 = SHORT A 16-bit (2 -byte) unsigned integer,
					else if (p.Type == ExifDataType.Short)// 0x3)
					{
					...
					else if (p.Type == ExifDataType.UInt)// 0x4)
					{
						// orientation // lookup table					
						v = convertToInt32U(p.Value).ToString();						
					}
						//5 = RATIONAL Two LONGs. The first LONG is the numerator and the second LONG expresses the//denominator.,
					else if( p.Type == ExifDataType.Rational)// 0x5 )
					{
						// rational
					...
					else if (p.Type == ExifDataType.Undefined)// 0x7)
					{					
						switch (p.Id )
						{ 
							case 0xA300:
					...
					else if (p.Type == ExifDataType.SLong)// 0x9)
					{
						v = convertToInt32(p.Value).ToString();
					}
						//10 = SRATIONAL Two SLONGs. The first SLONG is the numerator and the second SLONG is the
						//denominator.
					else if (p.Type == ExifDataType.SRational)// 0xA)
					{
					...
					// add it to the list
					if( !m_ExifData.ContainsKey(name) )
						m_ExifData.Add(name,v);
					// cat it too
					m_Data+= v;
					m_Data+= this.sp;
				}
			...
			public IEnumerator GetEnumerator()
			{
				...
				return( new ExifExtractorEnumerator( this.m_ExifData ));
			}
