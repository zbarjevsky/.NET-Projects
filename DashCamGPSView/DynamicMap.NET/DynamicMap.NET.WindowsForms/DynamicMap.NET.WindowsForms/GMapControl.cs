...
namespace DynamicMap.NET.WindowsForms
{
   ...
   using OpenNETCF.ComponentModel;
    ...
    public partial class GMapControl : UserControl, IDynMapInterface
    {
        ...
        /// </summary>
        public readonly ObservableCollectionThreadSafe<DynMapOverlay> Overlays = new ObservableCollectionThreadSafe<DynMapOverlay>();
      ...
      internal readonly Font MissingDataFont = new Font(FontFamily.GenericSansSerif, 8, FontStyle.Regular);
        ...
        public GMapControl()
        {
...
#endif
            {
                ...
                if (DynMaps.Instance.IsRunningOnMono)
                {
        ...
        void Overlays_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (DynMapOverlay obj in e.NewItems)
                {
                    if (obj != null)
                    {
                        obj.Control = this;
        ...
        internal void ForceUpdateOverlays()
        {
            try
            {
                ...
                foreach (DynMapOverlay o in Overlays)
                {
                    if (o.IsVisibile)
                    {
                        o.ForceUpdate();
        ...
        public void UpdateRouteLocalPosition(GMapRoute route)
        {
          ...
          {
                DynPoint p = FromLatLngToLocal(route.Points[i]);
        ...
        public void UpdatePolygonLocalPosition(GMapPolygon polygon)
        {
          ...
          {
                DynPoint p = FromLatLngToLocal(polygon.Points[i]);
#if !PocketPC
                if (!MobileMode)
                {
                    p.OffsetNegative(Core.renderOffset);
      ...
      protected override void OnPaintBackground(PaintEventArgs e)
      {
        ...
        void DrawGraphics(Graphics g)
        {
            ...
            if (MapRenderTransform.HasValue)
            {
                ...
                if (!MobileMode)
                {
                    var center = new DynPoint(Width / 2, Height / 2);
                    var delta = center;
                        ...
                        //if(Core.currentRegion.IntersectsWith(Core.tileRect) || IsRotated)
                        {
                            ...
                            else if (FillEmptyTiles && MapProvider.Projection is MercatorProjection)
                            {
                                ...
                                while (!parentTile.NotEmpty && zoomOffset < Core.Zoom && zoomOffset <= LevelsKeepInMemmory)
                                {
                                    ...
                                    parentTile = Core.Matrix.GetTileWithNoLock(Core.Zoom - zoomOffset++, new DynPoint((int)(tilePoint.PosXY.X / Ix), (int)(tilePoint.PosXY.Y / Ix)));
                                }
      ...
      protected override void OnResize(EventArgs e)
      {
                    ...
                    if (o != null && o.IsVisibile && o.IsHitTestVisible)
                    {
                        foreach (GMapMarker m in o.Markers)
                        {
                            if (m.IsVisible && m.IsHitTestVisible)
                            {
                                ...
                                DynPoint rp = new DynPoint(e.X, e.Y);
#if !PocketPC
                                if (!MobileMode)
                                {
        ...
        /// </summary>
        DynPoint ApplyRotationInversion(int x, int y)
        {
            DynPoint ret = new DynPoint(x, y);
        ...
        /// apply transformation if in rotation mode
        /// </summary>
        DynPoint ApplyRotation(int x, int y)
        {
            DynPoint ret = new DynPoint(x, y);
      ...
      public Size DragSize = new Size(4, 4);
...
#else
                DynPoint p = ApplyRotationInversion(e.X, e.Y);
#endif
        ...
        public GeoCoderStatusCode SetPositionByKeywords(string keys)
        {
            ...
            if (gp == null)
            {
                gp = MapProviders.OpenStreetMap as GeocodingProvider;
            }
        ...
        public void SerializeOverlays(Stream stream)
        {
            ...
            DynMapOverlay[] overlayArray = new DynMapOverlay[this.Overlays.Count];
        ...
        public void DeserializeOverlays(Stream stream)
        {
            ...
            DynMapOverlay[] overlayArray = BinaryFormatter.Deserialize(stream) as DynMapOverlay[];

            // Populate the collection of overlays.
            foreach (DynMapOverlay overlay in overlayArray)
            {
