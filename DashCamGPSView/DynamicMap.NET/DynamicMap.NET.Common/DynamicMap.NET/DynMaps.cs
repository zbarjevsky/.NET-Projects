...
namespace DynamicMap.NET
{
   ...
   /// maps manager
   /// </summary>
   public class DynMaps : Singleton<DynMaps>
   {
      /// <summary>
      /// tile access mode
      /// </summary>
      ...
      public bool ExportToGMDB(string file)
      {
         ...
         if(PrimaryCache is SQLitePureImageCache)
         {
            ...
            db.AppendFormat(CultureInfo.InvariantCulture, "{0}{1}Data.gmdb", DynMapProvider.LanguageStr, Path.DirectorySeparatorChar);
      ...
      {
#if SQLite
         if(PrimaryCache is DynamicMap.NET.CacheProviders.SQLitePureImageCache)
         {
            StringBuilder db = new StringBuilder((PrimaryCache as SQLitePureImageCache).GtileCache);
            db.AppendFormat(CultureInfo.InvariantCulture, "{0}{1}Data.gmdb", DynMapProvider.LanguageStr, Path.DirectorySeparatorChar);
      ...
      /// <returns></returns>
      public PureImage GetImageFrom(DynMapProvider provider, DynPoint pos, int zoom, out Exception result)
      {
         PureImage ret = null;
         result = null;
         ...
         try
         {
            ...
            if(UseMemoryCache)
            {
               ...
               if(m != null)
               {
                  if(DynMapProvider.TileImageProxy != null)
                  {
                     ret = DynMapProvider.TileImageProxy.FromArray(m);
                     if(ret == null)
                     {
#if DEBUG
