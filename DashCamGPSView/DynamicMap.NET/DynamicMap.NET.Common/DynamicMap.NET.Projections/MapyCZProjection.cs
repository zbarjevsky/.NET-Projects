...
namespace DynamicMap.NET.Projections
{
   ...
   public class MapyCZProjection : PureProjection
   {
      ...
      public override DynSize GetTileMatrixSizeXY(int zoom)
      {
         return new DynSize((long)Math.Pow(2, zoom), (long)Math.Pow(2, zoom));
      }

      public override DynSize GetTileMatrixSizePixel(int zoom)
      {
         DynSize s = GetTileMatrixSizeXY(zoom);
         return new DynSize(s.Width << 8, s.Height << 8);
      }

      public override DynSize GetTileMatrixMinXY(int zoom)
      {
         long wh = zoom > 3 ? (3 * (long)Math.Pow(2, zoom - 4)) : 1;
         return new DynSize(wh, wh);
      }
