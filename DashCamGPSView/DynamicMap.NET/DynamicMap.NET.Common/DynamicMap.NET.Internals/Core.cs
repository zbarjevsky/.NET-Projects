...
namespace DynamicMap.NET.Internals
{
   ...
   using Thread=OpenNETCF.Threading.Thread2;
    ...
    public class Core : IDisposable
    {
        ...
        public DynPoint renderOffset;
        public DynPoint centerTileXYLocation;
        public DynPoint centerTileXYLocationLast;
        public DynPoint dragPoint;
        public DynPoint compensationOffset;

        public DynPoint mouseDown;
        public DynPoint mouseCurrent;
        public DynPoint mouseLastZoom;
        ...
        public DynSize sizeOfMapArea;
        public DynSize minOfTiles;
        public DynSize maxOfTiles;

        public DynRect tileRect;
        public DynRect tileRectBearing;
        ...
        public DynMapProvider provider;
        public DynMapProvider Provider
        {
            get
            {
            ...
            set
            {
                if (provider == null || !provider.Equals(value))
                {
                    ...
                    {
                        tileRect = new DynRect(DynPoint.Empty, Provider.Projection.TileSize);
      ...
      public int LevelsKeepInMemmory = 1;
        ...
        public BackgroundWorker OnMapOpen()
        {
            if (!IsStarted)
            {
                ...
                if (x == 1)
                {
                    DynMaps.Instance.noMapInstances = false;
                }
        ...
        public void UpdateCenterTileXYLocation()
        {
            ...
            DynPoint centerPixel = Provider.Projection.FromLatLngToPixel(center, Zoom);
        ...
        /// <returns></returns>
        public DynPoint FromLatLngToLocal(PointLatLng latlng)
        {
            DynPoint pLocal = Provider.Projection.FromLatLngToPixel(latlng, Zoom);
        ...
        public int GetMaxZoomToFitRect(RectLatLng rect)
        {
            ...
            else
            {
                ...
                {
                    DynPoint p1 = Provider.Projection.FromLatLngToPixel(rect.LocationTopLeft, i);
                    DynPoint p2 = Provider.Projection.FromLatLngToPixel(rect.LocationRightBottom, i);
        ...
        /// </summary>
        /// <param name="pt"></param>
        public void BeginDrag(DynPoint pt)
        {
        ...
        internal void GoToCurrentPositionOnZoom()
        {
            ...
            if (MouseWheelZooming)
            {
                ...
                {
                    DynPoint pt = new DynPoint(-(positionPixel.X - Width / 2), -(positionPixel.Y - Height / 2));
        ...
        public void DragOffset(DynPoint offset)
        {
                        ...
                        do
                        {
                            ...
                            if (task.Zoom >= task.Core.provider.MinZoom && (!task.Core.provider.MaxZoom.HasValue || task.Zoom <= task.Core.provider.MaxZoom))
                            {
                                if (task.Core.skipOverZoom == 0 || task.Zoom <= task.Core.skipOverZoom)
                                {
                                    ...
                                    if (tl.InvertedAxisY)
                                    {
                                        img = DynMaps.Instance.GetImageFrom(tl, new DynPoint(task.Pos.X, task.Core.maxOfTiles.Height - task.Pos.Y), task.Zoom, out ex);
                                    }
                                    else // ok
                                    {
                                        img = DynMaps.Instance.GetImageFrom(tl, task.Pos, task.Zoom, out ex);
                                    }
                                }
                            }
                            ...
                            if (img == null && task.Core.okZoom > 0 && task.Core.fillEmptyTiles && task.Core.Provider.Projection is MercatorProjection)
                            {
                                ...
                                long Ix = 0;
                                DynPoint parentTile = DynPoint.Empty;

                                while (img == null && zoomOffset < task.Zoom)
                                {
                                    Ix = (long)Math.Pow(2, zoomOffset);
                                    parentTile = new DynamicMap.NET.DynPoint((task.Pos.X / Ix), (task.Pos.Y / Ix));
                                    img = DynMaps.Instance.GetImageFrom(tl, parentTile, task.Zoom - zoomOffset++, out ex);
                                }
        ...
        void OnLoadComplete(string ctid)
        {
            ...
            if (IsStarted)
            {
                DynMaps.Instance.MemoryCache.RemoveOverload();
            ...
            GC.Collect();
#endif
            Debug.WriteLine(ctid + " - OnTileLoadComplete: " + lastTileLoadTimeMs + "ms, MemoryCacheSize: " + DynMaps.Instance.MemoryCache.Size + "MB");
        ...
        void UpdateBounds()
        {
            ...
            try
            {
                ...
                for (long i = (int)Math.Floor(-sizeOfMapArea.Width * scaleX), countI = (int)Math.Ceiling(sizeOfMapArea.Width * scaleX); i <= countI; i++)
                {
                    ...
                    {
                        DynPoint p = centerTileXYLocation;
                        p.X += i;
                        p.Y += j;
               ...
               // ----------------------------
                        ...
                        if (p.X >= minOfTiles.Width && p.Y >= minOfTiles.Height && p.X <= maxOfTiles.Width && p.Y <= maxOfTiles.Height)
                        {
                            ...
                            {
                                PosXY = p,
                                PosPixel = new DynPoint(p.X * tileRect.Width, p.Y * tileRect.Height),
                ...
                if (DynMaps.Instance.ShuffleTilesOnLoad)
                {
...
#endif
                    DynMaps.Instance.noMapInstances = true;
                    DynMaps.Instance.WaitForCache.Set();
                    if (disposing)
                    {
                        DynMaps.Instance.MemoryCache.Clear();
                    }
                }
            }
        }
