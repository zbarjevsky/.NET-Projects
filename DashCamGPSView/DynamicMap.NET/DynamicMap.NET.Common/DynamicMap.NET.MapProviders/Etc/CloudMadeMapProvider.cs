...
namespace DynamicMap.NET.MapProviders
{
   ...
   public abstract class CloudMadeMapProviderBase : DynMapProvider, RoutingProvider, DirectionsProvider
   {
      ...
      DirectionsStatusCode GetDirectionsUrl(string url, out DynDirections direction)
      {
         ...
         try
         {
            string route = DynMaps.Instance.UseRouteCache ? Cache.Instance.GetContent(url, CacheType.DirectionsCache) : string.Empty;
            if(string.IsNullOrEmpty(route))
            {
               route = GetContentUsingHttp(url);
               if(!string.IsNullOrEmpty(route))
