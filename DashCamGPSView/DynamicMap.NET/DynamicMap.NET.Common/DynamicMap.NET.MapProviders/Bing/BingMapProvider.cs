...
namespace DynamicMap.NET.MapProviders
{
    ...
    public abstract class BingMapProviderBase : DynMapProvider, RoutingProvider, GeocodingProvider
    {
            ...
            if (!init)
            {
                try
                {
                    ...
                    if (TryGetDefaultKey && string.IsNullOrEmpty(ClientKey))
                    {
                        ...
                        key = "Anqg-XzYo-sBPlzOWFHIcjC3F8s17P_O7L4RrevsHVg4fJk6g_eEmUBphtSn4ySg"; // Stuff.EncriptedPasswordString("Jq7FrGTyaYqcrvv9ugBKv4OVSKnmzpigqZtdvtcDdgZexmOZ2RugOexFSmVzTAhOWiHrdhFoNCoySnNF3MyyIOo5u2Y9rj88");
                    }

                    #region -- try get sesion key --
                    if (!string.IsNullOrEmpty(key))
                    {
                        string keyResponse = DynMaps.Instance.UseUrlCache ? Cache.Instance.GetContent("BingLoggingServiceV1" + key, CacheType.UrlCache, TimeSpan.FromHours(DynMapProvider.TTLCache)) : string.Empty;
                        ...
                        if (!string.IsNullOrEmpty(keyResponse) && keyResponse.Contains("sessionId") && keyResponse.Contains("ValidCredentials"))
                        {
                            ...
                            Debug.WriteLine("MapProviders.BingMap.SessionId: " + SessionId);
                        }
                        else
                        {
                    ...
                    if (TryCorrectVersion && DisableDynamicTileUrlFormat)
                    {
                        ...
                        string html = DynMaps.Instance.UseUrlCache ? Cache.Instance.GetContent(url, CacheType.UrlCache, TimeSpan.FromDays(7)) : string.Empty;
                        ...
                        if (!string.IsNullOrEmpty(html))
                        {
                            ...
                            if (mat.Success)
                            {
                                ...
                                if (count == 2)
                                {
                                    ...
                                    string old = MapProviders.BingMap.Version;
                                    if (ver != old)
                                    {
                                        MapProviders.BingMap.Version = ver;
                                        MapProviders.BingSatelliteMap.Version = ver;
                                        MapProviders.BingHybridMap.Version = ver;
                                        MapProviders.BingOSMap.Version = ver;
#if DEBUG
                                        Debug.WriteLine("MapProviders.BingMap.Version: " + ver + ", old: " + old + ", consider updating source");
                                    ...
                                    {
                                        Debug.WriteLine("MapProviders.BingMap.Version: " + ver + ", OK");
                                    }
                                }
                            }
                            #endregion
        ...
        internal string GetTileUrl(string imageryType)
        {
            ...
            if (!string.IsNullOrEmpty(SessionId))
            {
                try
                {
                    ...
                    string r = DynMaps.Instance.UseUrlCache ? Cache.Instance.GetContent("GetTileUrl" + imageryType, CacheType.UrlCache, TimeSpan.FromDays(7)) : string.Empty;
                    bool cache = false;
                    ...
                    if (!string.IsNullOrEmpty(r))
                    {
                        ...
                        if (string.Compare(statuscode, "200", true) == 0)
                        {
                            ...
                            foreach (XmlNode xno in xnl)
                            {
                                ...
                                {
                                    if (cache && DynMaps.Instance.UseUrlCache)
                                    {
