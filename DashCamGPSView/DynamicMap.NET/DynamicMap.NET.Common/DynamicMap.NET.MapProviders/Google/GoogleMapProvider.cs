...
namespace DynamicMap.NET.MapProviders
{
    ...
    public abstract class GoogleMapProviderBase : DynMapProvider, RoutingProvider, GeocodingProvider, DirectionsProvider
    {
        ...
        public override void OnInitialized()
        {
            if (!init && TryCorrectVersion)
            {
                ...
                try
                {
                    string html = DynMaps.Instance.UseUrlCache ? Cache.Instance.GetContent(url, CacheType.UrlCache, TimeSpan.FromHours(DynMapProvider.TTLCache)) : string.Empty;
                    ...
                    if (string.IsNullOrEmpty(html))
                    {
                        ...
                        if (!string.IsNullOrEmpty(html))
                        {
                            if (DynMaps.Instance.UseUrlCache)
                            {
        ...
        List<PointLatLng> GetRoutePoints(string url, int zoom, out string tooltipHtml, out int numLevel, out int zoomFactor)
        {
            ...
            try
            {
                string route = DynMaps.Instance.UseRouteCache ? Cache.Instance.GetContent(url, CacheType.RouteCache) : string.Empty;
                ...
                if (string.IsNullOrEmpty(route))
                {
                    ...
                    if (!string.IsNullOrEmpty(route))
                    {
                        if (DynMaps.Instance.UseRouteCache)
                        {
        ...
        GeoCoderStatusCode GetLatLngFromGeocoderUrl(string url, out List<PointLatLng> pointList)
        {
            ...
            try
            {
                string geo = DynMaps.Instance.UseGeocoderCache ? Cache.Instance.GetContent(url, CacheType.GeocoderCache) : string.Empty;
                    ...
                    if (geo.StartsWith("<?xml"))
                    {
                        ...
                        if (nn != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (cache && DynMaps.Instance.UseGeocoderCache)
                                {
        ...
        GeoCoderStatusCode GetPlacemarkFromReverseGeocoderUrl(string url, out List<Placemark> placemarkList)
        {
            ...
            try
            {
                string reverse = DynMaps.Instance.UsePlacemarkCache ? Cache.Instance.GetContent(url, CacheType.PlacemarkCache) : string.Empty;
                    ...
                    if (reverse.StartsWith("<?xml"))
                    {
                        ...
                        if (nn != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (cache && DynMaps.Instance.UsePlacemarkCache)
                                {
        ...
        public DirectionsStatusCode GetDirections(out DynDirections direction, PointLatLng start, PointLatLng end, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
        {
        ...
        public DirectionsStatusCode GetDirections(out DynDirections direction, string start, string end, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
        {
            return GetDirectionsUrl(MakeDirectionsUrl(start, end, LanguageStr, avoidHighways, avoidTolls, walkingMode, sensor, metric), out direction);
        }
        ...
        /// <returns></returns>
        public IEnumerable<DynDirections> GetDirections(out DirectionsStatusCode status, string start, string end, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
        {
        ...
        /// <param name="metric"></param>
        /// <returns></returns>
        public IEnumerable<DynDirections> GetDirections(out DirectionsStatusCode status, PointLatLng start, PointLatLng end, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
        {
            // TODO: add alternative directions
        ...
        public DirectionsStatusCode GetDirections(out DynDirections direction, PointLatLng start, IEnumerable<PointLatLng> wayPoints, PointLatLng end, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
        {
        ...
        public DirectionsStatusCode GetDirections(out DynDirections direction, string start, IEnumerable<string> wayPoints, string end, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
        {
            return GetDirectionsUrl(MakeDirectionsUrl(start, wayPoints, end, LanguageStr, avoidHighways, avoidTolls, walkingMode, sensor, metric), out direction);
        }
        ...
        DirectionsStatusCode GetDirectionsUrl(string url, out DynDirections direction)
        {
            ...
            try
            {
                string kml = DynMaps.Instance.UseDirectionsCache ? Cache.Instance.GetContent(url, CacheType.DirectionsCache) : string.Empty;
                ...
                if (!string.IsNullOrEmpty(kml))
                {
                    ...
                    if (nn != null)
                    {
                        ...
                        {
                            if (cache && DynMaps.Instance.UseDirectionsCache)
                            {
                                Cache.Instance.SaveContent(url, CacheType.DirectionsCache, kml);
                            }

                            direction = new DynDirections();
