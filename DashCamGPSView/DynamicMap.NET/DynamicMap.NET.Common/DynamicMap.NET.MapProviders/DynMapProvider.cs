...
namespace DynamicMap.NET.MapProviders
{
    ...
    public class MapProviders
    {
        static MapProviders()
        {
            list = new List<DynMapProvider>();

            Type type = typeof(MapProviders);
            foreach (var p in type.GetFields())
            {
                var v = p.GetValue(null) as DynMapProvider; // static classes cannot be instanced, so use null...
                if (v != null)
                {
                    list.Add(v);
                }
            ...
            Hash = new Dictionary<Guid, DynMapProvider>();
            foreach (var p in list)
            {
                Hash.Add(p.Id, p);
            }

            DbHash = new Dictionary<int, DynMapProvider>();
            foreach (var p in list)
            {
                DbHash.Add(p.DbId, p);
            }
      ...
      public static readonly OpenStreetMapSurferTerrainProvider OpenStreetMapSurferTerrain = OpenStreetMapSurferTerrainProvider.Instance;
        ...
        static List<DynMapProvider> list;

        /// <summary>
        /// get all instances of the supported providers
        /// </summary>
        public static List<DynMapProvider> List
        {
            get
            {
                return list;
        ...
        static Dictionary<Guid, DynMapProvider> Hash;

        public static DynMapProvider TryGetProvider(Guid id)
        {
            DynMapProvider ret;
        ...
        static Dictionary<int, DynMapProvider> DbHash;

        public static DynMapProvider TryGetProvider(int DbId)
        {
            DynMapProvider ret;
    ...
    /// </summary>
    public abstract class DynMapProvider
    {
        /// <summary>
        ...
        /// provider overlays
        /// </summary>
        public abstract DynMapProvider[] Overlays
        {
            get;
        }
        ...
        /// <returns></returns>
        public abstract PureImage GetTileImage(DynPoint pos, int zoom);

        static readonly List<DynMapProvider> MapProviders = new List<DynMapProvider>();

        protected DynMapProvider()
        {
      ...
      public static int TimeoutMs = 44 * 1000; 
        ...
        protected static int GetServerNum(DynPoint pos, int max)
        {
        ...
        {
            if (obj is DynMapProvider)
            {
                return Id.Equals((obj as DynMapProvider).Id);
            }
            return false;
        }        
    ...
    /// </summary>
    public class EmptyProvider : DynMapProvider
    {
