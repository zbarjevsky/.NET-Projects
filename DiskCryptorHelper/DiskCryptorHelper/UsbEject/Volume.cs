...
namespace UsbEject.Library
{
    ...
    public class Volume : Device, IComparable
    {
            ...
            get
            {
                if (_disks == null)
                {
                    ...
                    if (DiskNumbers != null)
                    {
                        ...
                        {
                            foreach (Device disk in disks.GetDevices())
                            {
        ...
        {
            string friendlyName = this.VolumeName;
            if (Disks.Count > 0)
                friendlyName = Disks[0].FriendlyName;

            string removable = this.RemovableDevices.Count > 0 ? "Yes" : "No";
            return LogicalDrive + ", Removable: " + removable + " [" + friendlyName + "]";
        }
    }
}
